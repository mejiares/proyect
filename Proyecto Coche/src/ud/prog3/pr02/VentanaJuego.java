...
public class VentanaJuego extends JFrame {
	...
	JLabel lMensaje = new JLabel ("Que pasa!!");
	int perdidas = 0;
	int puntuacion = 0;
	MiRunnable miHilo = null;
	// Hilo del bucle principal de juego	
	int crea = 0;
	double cont= 0;
	boolean[] Arraybool = new boolean[4];
	...
	if(	Arraybool[0]==true ){
		...
		//System.out.println(miMundo.creacion());
	...
	class MiRunnable implements Runnable {
		...
		public void run() {
			...
			while (sigo) {
				...
				lMensaje.setText("Estrellas perdidas:"+perdidas+"   Puntuacion: "+puntuacion);
				if(perdidas == 10){
					lMensaje.setText("El juego a terminado tu puntuacion ha sido: "+puntuacion);
					acaba();
				}
				try {
					Thread.sleep( 40 );
